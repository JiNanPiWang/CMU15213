...
```
照样是读取六个数，然后把第一个参数放入eax（32位），要保证eax-1之后还要大于5
...
```
movslq是有符号扩展，简单的说就是rax=ebx，循环1到5。然后让eax=(%rsp+4*%rax)，大胆猜一下rax应该是1~5循环。并且第2~6个输入不能与第一个输入相同，否则爆炸。
...
```
r13=rsp+4，指向第二个输入，跳回401114，看了一下，意思大概是每个都不能相同，且都要大于6。
...
```
rsi初始为rsp+24，rax是rsp，栈顶元素-=7，rax+=4，rax不等于rsp+24就继续。跳出时，栈顶元素已经减了42.
...
```
循环：esi=0，如果1<=第1~6个输入，让(rsp+32+2*rsi)=0x6032d0，rsi+=4，如果rsi != 24，就继续
